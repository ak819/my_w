            ...
            <div class="col-lg-12 col-md-12 col-sm-12 col-12">
                <div class="float-left mt-2">
                    <div class="d-inline-flex align-items-center">
                        <h1>Users</h1><span class="font-size-16 font-wt-600 color-gray">{{ count($users) }} Total</span>
                        <ol class="bclink">
                ...
                <div class="float-right">
                    <a href="{{ route('users.create') }}" class="btn btn-medium btn-gold ml-2 mr-2 float-right">Add User</a>
                </div>
            </div>
        </div>
    </div>
                   ...
                   <thead>
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            @if(count($users->UserPropertyType) > 0)
                         
                            <td>@php
                                $i=1;   
                              foreach($propetyTypes as $val)
                              {  
                                 if(in_array($val->ID,$users->UserPropertyType))
                                 {
                                    echo $i.' .'.$val->Name.'<br>';
                                 }
                            ...
                            <td>{{datetime_format($users->created_at)}}</td>
                             <td>
                                @if($users->IsEnable == 1)
                                ...
                                <td>
                                    <form action="{{ route('users.destroy',$users->guid)}}" method="post">
                                        <a href="{{ route('users.edit',$users->guid) }}" class="btn btn-small btn-gold-lt" title="Edit Details"><i class="material-icons icon-2x">edit</i></a>
                                        <a href="{{ route('change.user.password',$users->guid) }}" class="btn btn-small btn-red-lt userdetails"><i class="material-icons icon-2x">lock_open</i></a>
                                        @method('delete')
                                        @csrf
