                ...
                <div class="form-floating">
                    ...
                    <select name="" class="form-select select2 locationlist" multiple="multiple" aria-label="Floating label select example">
                        @if ($Data->Locations->first())
                            ...
                            <option value="{{ $location->ID }}" data-char="{{ strtolower(preg_replace('/[^A-Za-z0-9\-]/', '-', $location->CommunityNameEng)) }}" data-cityid="{{ $location->CityID }}"  @php if($locations && in_array($location->ID, $locations)){ $LocationsName.=$location->CommunityName.','; echo"selected"; }  @endphp>{{ $location->CommunityName }}</option>
                            @endforeach
                        @endif
                    </select>
