    ...
    <div class="container animate__animated animate__fadeIn animate__slow">
        <ul class="nav nav-pills mb-3" id="pills-tab" role="tablist">
            <li class="nav-item" role="presentation">
                <a class="nav-link  @if(app('request')->input('adt') == '2') active @endif" href="{{ route('search-properties',[app()->getLocale(),'sale']) }}">{{ __('msg.Sale') }}</a>
            </li>
            <li class="nav-item" role="presentation">
                <a class="nav-link  @if(app('request')->input('adt') == '1') active @endif" href="{{ route('search-properties',[app()->getLocale(),'rent']) }}">{{ __('msg.Rent') }}</a>
            </li>
        </ul>
        ...
        <form id="searchform" action="{{ route('search-properties',[app()->getLocale(),'sale']) }}" method="get">
            ...
            @else
            <form id="searchform" action="{{ route('search-properties',[app()->getLocale(),'rent']) }}" method="get">
            <input type="hidden" id="form_url" value="{{ route('search-properties',[app()->getLocale(),'rent','','']) }}">
            @endif
        <div class="row g-3">
           ...
           <div class="col-xl-3 col-lg-2 col-md-6 col-sm-6 col-12">
            <div class="form-floating">
                ...
                <select name="t" id="propertyType" class="form-select urlfilters" aria-label="Floating label select example">
                    ...
                    @else
                    <option value="{{ $unitypes->ID }}" data-char="{{ strtolower(preg_replace('/[^A-Za-z0-9\-]/', '-',$unitypes->TypeNameEng)) }}" data-filterid="{{ $unitypes->FilterDivId }}" @php if (app('request')->input('t') == $unitypes->ID){ $PropertyTypeName.=$unitypes->TypeName;  echo "selected";  } @endphp>{{ $unitypes->TypeName }}</option>
                    @endif
                    @endforeach
                </select>
            ...
            <div class="col-xl-6 col-lg-5 col-md-6 col-sm-6 col-12">
                <div class="form-floating">
                    ...
                    $LocationsName="";
                    @endphp
                    <select name="" class="form-select select2 locationlist urlfilters" multiple="multiple" aria-label="Floating label select example">
            ...
            <div class="col-xl-2 col-lg-2 col-md-6 col-sm-6 col-12">
                ...
                <input type="hidden"  name="adt" value="{{ app('request')->input('adt') }}">
            ...
            <div class="col-xl-12 col-lg-4 col-md-4 col-sm-4 col-12 mb-2">
                <div class="float-end">
                <a class="btn btn-outline-white"  @if(!app('request')->input('t')) style="display:none" @endif  data-bs-toggle="collapse" id="advance-search" href="#collapseExample" role="button" aria-expanded="false" aria-controls="collapseExample">
        ...
        <div class="row">
            <div class="col-xl-7 col-lg-7 col-md-5 col-sm-5 col-12">
                <h1 class="font-size-18 font-wt-600 animate__animated animate__fadeIn animate__slow">{{$Data->Count }} 
                    ...
                    @endif 
                    @if ($LocationsName)
                    {{  __('msg.in') }} {{ $LocationsName = rtrim($LocationsName, ", "); }}
                    @endif
                    @endif
                    @if(app('request')->input('adt') == "1")
                    @if($PropertyTypeName)
                    {{ $PropertyTypeName }}
                    {{ __('msg.ForRent') }}
                    @else
                    {{  __('msg.propertiesforrent') }}
                    @endif
