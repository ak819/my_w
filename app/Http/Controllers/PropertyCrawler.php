            ...
            $CronAnalysisID=DB::table('cronanalysis')->insertGetId($Crondata);
                ...
                while($IsXmlLoaded==0)
                {
                    ...
                    try {
                        ...
                        $XMLDoc = simplexml_load_string($res);
                        $IsXmlLoaded=1;
                    } catch (\Throwable $th) {
                        continue;
                    }
             ...
             if(empty(!$Propety) && $Propety->ID!==" ")
             {
               ...
               //if(strtotime($LastUpdate) > strtotime($Propety->CRMLastUpdateDate))
                //{
                    ...
                    'PropertyTitleAr'=>$Record->Property_Title_Ar,
                    'Slug'=>generateSlug('properties','PropertyTitle',$PropertyTitle),
                    'SlugAr'=>generateSlugAr('properties','PropertyTitleAr',$PropertyTitleAr),
    ...
    if(empty(!$Images))
    {
       ...
       { 
           ...
           try {
                ...
                $FilePath = 'uploads/properties/orignal/'.trim($val->PropertyRefNo).'/'.trim($val->FileName);
                $img = Image::make($FilePath);
                $img->save($FilePath,60);
                $data=['IsOptimized'=>1];
                DB::table('property_images')->where('Guid',$val->Guid)->update($data);
                }else{
                $FilePath = 'uploads/properties/thumb/'.trim($val->PropertyRefNo).'/'.trim($val->FileName);
                $img = Image::make($FilePath);
                $img->save($FilePath,60);
                $data=['IsOptimized'=>1];
                DB::table('property_images')->where('Guid',$val->Guid)->update($data);
